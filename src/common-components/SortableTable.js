...
export default class SortableTable extends Component {
    ...
    render() {
        const {
            ...
            paginationClassName, paginationInline, responsive
        } = this.props;
        const {sort} = this.state;
                                    ...
                                    <tr key={key} onClick={() => this.onSelectRow(row)}
                                        ...
                                        {fields.map((field, index) => {
                                            if (typeof field.render === 'function') {
                                                return (<td key={index}
                                                            className={field.className}>{field.render(row)}</td>);
                                            }
                                            return (
                                                <td key={index} className={field.className}>{row[field.field] === null ?
                                                    <span
                                                        className="null-content">&#x2400;</span> : row[field.field]}</td>);
                                        })}
                                    </tr>
                                )
                            })
